...
# RAG Расписание (для контроля расходов Google API)
ENABLE_RAG_SCHEDULE=false            # Включить/выключить автоматический запуск RAG задач
RAG_MATCHER_INTERVAL_MINUTES=360     # Интервал запуска matcher (по умолчанию 6 часов)
RAG_DEDUP_HOUR=3                     # Час запуска deduplicator (по умолчанию 3:00 ночи)

# Go Backend Server
GO_SERVER_API_ENDPOINT=http://localhost:8080/internal/worker
...
#### Способ 1: Автоматический запуск (использует настройки из .env)

```bash
make start-all
```

Эта команда автоматически определяет режим на основе `ENABLE_RAG_SCHEDULE` в `.env`:
- Если `ENABLE_RAG_SCHEDULE=false` (по умолчанию) — RAG задачи НЕ запускаются
- Если `ENABLE_RAG_SCHEDULE=true` — RAG задачи запускаются по расписанию

#### Способ 1А: Запуск БЕЗ RAG расписания (экономия средств) 💰
...
Что работает:
- ✅ Gemini Worker (парсинг тендеров)
- ✅ Celery Beat (планировщик для cleanup задач)
- ❌ RAG Matcher НЕ запускается автоматически
- ❌ RAG Deduplicator НЕ запускается автоматически

#### Способ 1Б: Запуск С RAG расписанием (полная функциональность) 💸

**ВНИМАНИЕ: Тратит деньги на Google API!**
...
- ✅ RAG Matcher запускается по расписанию
- ✅ RAG Deduplicator запускается по расписанию

Для включения RAG задач установите в `.env`:
```bash
ENABLE_RAG_SCHEDULE=true
# Настройте интервалы для контроля расходов:
RAG_MATCHER_INTERVAL_MINUTES=360  # По умолчанию 6 часов
...
```bash
# Терминал 1: Celery Worker (только AI задачи)
celery -A app.celery_app worker --loglevel=INFO --queues=ai_queue --concurrency=1

# Терминал 2: Celery Beat (опционально)
# ⚠️ ВНИМАНИЕ: Beat запускает RAG задачи, которые тратят деньги на Google API!
# Для отключения автозапуска RAG задач установите в .env:
# ENABLE_RAG_SCHEDULE=false
# Для изменения интервалов (если включено):
# RAG_MATCHER_INTERVAL_MINUTES=360  (по умолчанию 6 часов)
# RAG_DEDUP_HOUR=3                  (по умолчанию 3:00 ночи)
...
### ⚙️ Управление RAG расписанием

RAG воркеры используют Google API и **тратят деньги**! Контролируйте их запуск через настройки в `.env`:

#### Вариант А: Полное отключение (рекомендуется для разработки)
```env
ENABLE_RAG_SCHEDULE=false  # RAG задачи НЕ запускаются автоматически
```
- ✅ Безопасно для разработки
- ✅ Не тратит деньги на Google API
- ℹ️ RAG задачи можно запускать вручную через API или Celery

#### Вариант Б: Редкий запуск (экономия средств)
```env
ENABLE_RAG_SCHEDULE=true
RAG_MATCHER_INTERVAL_MINUTES=1440  # 1 раз в день (24 часа)
RAG_DEDUP_HOUR=3                    # В 3:00 ночи
```
- 💰 Минимальные расходы на API
- 🕐 Matcher: 1 раз в день
- 🕐 Deduplicator: 1 раз в день в 3:00

#### Вариант В: Частый запуск (для production)
```env
ENABLE_RAG_SCHEDULE=true
RAG_MATCHER_INTERVAL_MINUTES=60    # Каждый час
RAG_DEDUP_HOUR=3                    # В 3:00 ночи
```
- ⚡ Быстрая обработка новых позиций
- 💸 Повышенные расходы на Google API
- 🕐 Matcher: каждый час
- 🕐 Deduplicator: 1 раз в день в 3:00

> **Примечание:** По умолчанию `ENABLE_RAG_SCHEDULE=false`, RAG задачи отключены.
...
# ============== RAG Расписание (контроль расходов Google API) ==============
ENABLE_RAG_SCHEDULE=false            # Включить/выключить автозапуск RAG задач
RAG_MATCHER_INTERVAL_MINUTES=360     # Интервал запуска matcher (по умолчанию 6 часов)
RAG_DEDUP_HOUR=3                     # Час запуска deduplicator (по умолчанию 3:00 ночи)

# ⚠️ ВАЖНО: RAG воркеры используют Google API и тратят деньги!
# - При ENABLE_RAG_SCHEDULE=false задачи запускаются только вручную
# - При ENABLE_RAG_SCHEDULE=true задачи запускаются автоматически по расписанию
# - Увеличьте RAG_MATCHER_INTERVAL_MINUTES для снижения расходов (напр. 1440 = 1 раз в день)
