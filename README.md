...
**Система автоматически читает конфигурацию из `.env`:**

Поведение определяется переменной `ENABLE_RAG_SCHEDULE`:
- `ENABLE_RAG_SCHEDULE=false` (по умолчанию) — **RAG задачи отключены** 💰
  - ✅ Gemini Worker работает
  - ✅ Celery Beat планировщик работает
  - ❌ RAG задачи НЕ запускаются автоматически
  - 💰 **Не тратит деньги на Google API**

- `ENABLE_RAG_SCHEDULE=true` — **RAG задачи включены** 💸
  - ✅ Gemini Worker работает
  - ✅ Celery Beat планировщик работает
  - ✅ RAG Matcher запускается по расписанию
  - ✅ RAG Deduplicator запускается по расписанию
  - 💸 **Тратит Google API согласно расписанию**

**Неинтерактивный режим (для CI/CD):**
```bash
NO_PROMPT=true ./scripts/start_services.sh
```
...
# Поведение зависит от ENABLE_RAG_SCHEDULE в .env
