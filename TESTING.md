...
Рекомендуемый способ подключить директорию фикстур в тестах:
...
@pytest.fixture(scope="session")
def excel_fixtures(fixtures_dir) -> Path:
    return fixtures_dir / "excel"

@pytest.fixture(scope="session")
def json_fixtures(fixtures_dir) -> Path:
    return fixtures_dir / "json"

@pytest.fixture(scope="session")
def gemini_fixtures(fixtures_dir) -> Path:
