...
class LLMProcessor:
    ...
    async def process_batch(self, session: aiohttp.ClientSession, batch_text: str, batch_num: int) -> Optional[SlurryWallSpecs]:
        """
        ...
        for attempt in range(self.settings.max_retries):
            try:
                async with session.post(self.settings.ollama_url, json=payload, headers=self.headers, timeout=self.settings.timeout) as response:
                    ...
                    if validated_response and validated_response.slurry_wall_specs and any(v is not None for v in validated_response.slurry_wall_specs.model_dump().values()):
                        logging.info(
...
async def main():
    ...
    if not settings.input_file.exists():
        ...
        print(json.dumps({"slurry_wall_specs": {}},
    ...
    async with aiohttp.ClientSession() as session:
        ...
        logging.info(
            f"Отправка {len(tasks)} батчей на асинхронную обработку...")
        ...
        for i, result in enumerate(batch_results):
            if isinstance(result, Exception):
                ...
                logging.info(
                    f"Батч {i+1}: Найдены параметры: {result.model_dump()}")
            else:
                logging.info(
