...
3. **Бинарные файлы** (`.xlsx`) и golden JSON (`.json`): должны коммититься в репозиторий «как есть», чтобы тесты были воспроизводимыми. Корневой `.gitignore` содержит глобальные правила `*.xlsx` и `*.json`, поэтому в нём добавлены явные исключения `!tests/fixtures/**/*.xlsx` и `!tests/fixtures/**/*.json`. Не удаляйте эти исключения. Для генерации синтетических файлов используйте скрипты в `scripts/generate_fixtures.py` (если появится).
