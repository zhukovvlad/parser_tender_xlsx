...
def create_hierarchical_report(
    positions_data: dict, output_filename: str, lot_name: str
):
    """
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Анализ файла тендера в формате JSON и создание отчетов по лотам."
    )
    ...
    for lot_key, lot_info in lots_data.items():
        ...
        if not contractor_data:
            print(
                f"  - ⚠️ Не найден подрядчик '{contractor_key_to_process}' в лоте '{lot_name}'. Пропускаю."
            )
            continue
        ...
        if not positions:
            print(
                f"  - ⚠️ Не найдено позиций у подрядчика '{contractor_key_to_process}' в лоте '{lot_name}'."
            )
            continue
