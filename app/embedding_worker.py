...
def process_positions(engine: Engine, model: SentenceTransformer, update_all: bool = False) -> int:
    """
    ...
    if update_all:
        ...
        query = text("SELECT id, standard_job_title, description FROM catalog_positions")
    else:
    ...
    try:
        with engine.begin() as connection:
            ...
            embeddings: np.ndarray = model.encode(texts_to_embed, show_progress_bar=True, normalize_embeddings=True)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Воркер для создания эмбеддингов для позиций в каталоге.")
    parser.add_argument(
        "--update-all",
