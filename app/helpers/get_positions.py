...
def get_positions(
    ws: Worksheet, contractor: Dict[str, Any]
    ...
    while True:
        ...
        current_row_tuple = ws[
            current_row_num
        ]  # Кортеж ячеек (Cell) для текущей строки
        ...
        if mode == "normal":
            ...
            for merged_range_obj in ws.merged_cells.ranges:
                if (
                    merged_range_obj.min_row
                    <= first_cell_in_row.row
                    <= merged_range_obj.max_row
                    and merged_range_obj.min_col
                    <= first_cell_in_row.column
                    <= merged_range_obj.max_col
                ):
        ...
        if mode == "normal":
            ...
            item[JSON_KEY_JOB_TITLE_NORMALIZED] = (
                normalize_job_title_with_lemmatization(original_job_title)
            )
            item[JSON_KEY_COMMENT_ORGANIZER] = ws.cell(
                row=current_row_num, column=6
            ).value
