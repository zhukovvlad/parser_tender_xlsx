...
try:
    ...
    print(
        "ПРЕДУПРЕЖДЕНИЕ: Библиотека spaCy не найдена. "
    ...
    print(
        "ПРЕДУПРЕЖДЕНИЕ: Модель spaCy 'ru_core_news_sm' не найдена. "
        "Пожалуйста, скачайте ее командой: python -m spacy download ru_core_news_sm. "
        "Лемматизация spaCy будет пропущена."
    )
except Exception as e:
    print(
        f"ПРЕДУПРЕЖДЕНИЕ: Непредвиденная ошибка при инициализации spaCy: {e}. "
        "Лемматизация spaCy будет пропущена."
    )
# --- Конец блока spaCy ---
...
def normalize_job_title_with_lemmatization(text: Optional[str]) -> Optional[str]:
    ...
    processed_text_for_join = (
        cleaned_text  # Значение по умолчанию, если лемматизация не сработает
    )
    ...
    if SPACY_AVAILABLE and NLP_SPACY:
        try:
            ...
            for token_idx, token in enumerate(doc):
                ...
                if (
                    not token.is_punct and not token.is_space and token.lemma_
                ):  # Проверяем, что лемма не пустая
                    lemmatized_words.append(
                        token.lemma_
                    )  # spaCy леммы обычно уже в нижнем регистре
            ...
            print(
                f"ПРЕДУПРЕЖДЕНИЕ: Ошибка во время лемматизации с spaCy для текста '{cleaned_text[:50]}...': {e}"
            )
            print(
                "ПРЕДУПРЕЖДЕНИЕ: Лемматизация для этого текста будет пропущена, используется текст после базовой очистки."
            )
