...
JSON_KEY_SUGGESTED_QUANTITY = (
    "suggested_quantity"  # Количество, предложенное участником (если отличается от объема организатора)
)
...
JSON_KEY_CONTRACTOR_INDEX = (
    "contractor_"  # Префикс для автоматической генерации ключей отдельных подрядчиков (например, "contractor_1")
)
...
JSON_KEY_CONTRACTOR_ITEMS = (
    "contractor_items"  # Общий ключ для JSON-объекта, содержащего все позиции и итоги данного подрядчика
)
...
JSON_KEY_BASELINE_PROPOSAL = (
    "baseline_proposal"  # Используется в постобработке для структурирования "Расчетной стоимости"
)
