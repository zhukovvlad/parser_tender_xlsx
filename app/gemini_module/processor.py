...
class TenderProcessor:
    ...
    def upload(self, file_path: str):
        ...
        print(get_message("file_uploaded", file_id=self.file.name))
        return self
    ...
    def classify(self, categories: list[str], fallback_label: str = "не найдено") -> str:
        ...
        if result not in categories and result != fallback_label:
            print(get_message("invalid_category", result=result))
    ...
    def delete_uploaded_file(self):
        ...
        if not self.file:
            print("ℹ️ Нет загруженного файла для удаления.")
            return
        try:
            print(f"⏳ Удаляю файл {self.file.name} с сервера...")
            self.client.files.delete(name=self.file.name)
            print(get_message("file_deleted", file_id=self.file.name))
            self.file = None
        except Exception as e:
            print(get_message("cleanup_error", error=str(e)))
