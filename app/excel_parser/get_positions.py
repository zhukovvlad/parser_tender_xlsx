...
from ..constants import JSON_KEY_INITIAL_COST  # Используется и для ключа в summary, и для поиска по тексту
from ..constants import TABLE_PARSE_DEVIATION_FROM_CALCULATED_COST  # Текстовый маркер для поиска "отклонение..."
from ..constants import TABLE_PARSE_INITIAL_COST  # Текстовый маркер для поиска "первоначальная..."
from ..constants import (
    ...
    while True:
        ...
        if mode == "normal":
            ...
            for merged_range_obj in ws.merged_cells.ranges:
                if (
                    merged_range_obj.min_row <= first_cell_in_row.row <= merged_range_obj.max_row
                    and merged_range_obj.min_col <= first_cell_in_row.column <= merged_range_obj.max_col
                ):
        ...
        if mode == "normal":
            ...
            summary_label_raw = (
                str(first_cell_in_row.value).strip().lower() if first_cell_in_row.value is not None else ""
            )
