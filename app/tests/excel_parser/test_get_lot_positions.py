...
def sample_worksheet():
    ...
    # Заголовки (строка 1)
    headers = [
        "№ п/п", "Глава", "Артикул СМР", "Наименование видов работ", "Пропуск", 
        "Комментарий", "Ед. изм.", "Кол-во", "Подрядчик 1 - Цена", "Подрядчик 1 - Стоимость"
    ]
...
class TestGetLotPositionsBehavior:
    ...
    def test_extracts_positions_from_sample_data(self, sample_worksheet):
        ...
        result = get_lot_positions(
            sample_worksheet, 
            contractor, 
            lot_start_row=13,  # Первая позиция 
            lot_end_row=15     # Последняя позиция
        )
    ...
    def test_handles_empty_range_gracefully(self, sample_worksheet):
        ...
        result = get_lot_positions(
            sample_worksheet, 
            contractor, 
            lot_start_row=20,  # После наших данных
            lot_end_row=15     # Невалидный диапазон
        )
    ...
    def test_respects_lot_boundaries(self, sample_worksheet):
        ...
        result = get_lot_positions(
            ...
            contractor, 
            lot_start_row=13,  # Первая позиция
            lot_end_row=14     # Только первые 2 строки
        )
    ...
    def test_processes_contractor_data_correctly(self, sample_worksheet):
        ...
        """
        contractor = {
            "column_start": 9,  # Колонка "Подрядчик 1 - Цена"
            "merged_shape": {"colspan": 8}
        }
        ...
        result = get_lot_positions(
            ...
            contractor, 
            lot_start_row=13,  # Первая позиция
            lot_end_row=13     # Только первая позиция
        )
    ...
    @pytest.mark.skipif(not Path(__file__).parent.parent.joinpath("test_data", "sample_tender.xlsx").exists(),
    ...
    def test_integration_with_real_excel_data(self, real_excel_file):
        ...
        contractor = {
            "column_start": 9,  # Примерная колонка подрядчика
            "merged_shape": {"colspan": 8}
        }
...
class TestGetLotPositionsEdgeCases:
    ...
    def test_extreme_row_ranges(self, sample_worksheet):
        ...
        result = get_lot_positions(
            ...
            contractor, 
            lot_start_row=13,
            lot_end_row=5000  # Реалистичный размер для крупного тендера
        )
