                ...
                async with session.post(
                    ...
                    try:
                        ...
                        try:
                            ...
                            wrapped_data = {"slurry_wall_specs": SlurryWallSpecs(**extracted_data)}
                    ...
                    if (
                        ...
                        and any(v is not None for v in validated_response.slurry_wall_specs.model_dump().values())
                    ):
                        logging.info(
                        ...
                        logging.warning(f"Батч {batch_num}: Данные не содержат параметров стены в грунте или пусты.")
                        return None
...
def merge_slurry_wall_specs(batch_results: List[Optional[SlurryWallSpecs]]) -> dict:
    ...
    for result in batch_results:
        ...
        if not isinstance(result, SlurryWallSpecs) or not any(v is not None for v in result.model_dump().values()):
            continue
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for i, result in enumerate(batch_results):
            if isinstance(result, Exception):
                logging.error(f"Батч {i+1}: Во время выполнения задачи произошла ошибка: {result}")
            elif isinstance(result, SlurryWallSpecs) and any(v is not None for v in result.model_dump().values()):
                logging.info(f"Батч {i+1}: Найдены параметры: {result.model_dump()}")
            else:
                logging.info(f"Батч {i+1}: Полезные данные не найдены или произошла ошибка при обработке.")

    filtered_results = [r for r in batch_results if isinstance(r, (SlurryWallSpecs, type(None)))]
