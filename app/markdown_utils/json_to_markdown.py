...
def generate_markdown_for_lots(data: Dict[str, Any]) -> Tuple[Dict[str, List[str]], Dict[str, Optional[str]]]:
    """
    ...
    initial_metadata: Dict[str, Optional[str]] = {
        "tender_id": tender_id_val, "tender_title": title_val, "tender_object": obj_val,
        "tender_address": addr_val, "executor_name": exec_name_s, "executor_phone": exec_phone_s,
        "executor_date": exec_date_s
    }
    initial_metadata = {k: v for k, v in initial_metadata.items() if v is not None and (str(v).strip() != "" or k == "tender_id")}
    ...
    for lot_key_str, lot_data_dict in data.get(JSON_KEY_LOTS, {}).items():
                        ...
                        if isinstance(total_cost_s_data, dict) and any(v is not None for v in total_cost_s_data.values()):
                            ...
                            for k_cost, v_cost in total_cost_s_data.items():
                                if v_cost is not None:
                                    lot_specific_md_lines.append(f"  - {sanitize_text(k_cost).capitalize()}: {v_cost} руб.")
                if not has_baseline_output:
                    lot_specific_md_lines.append(f"  *Итоговые суммы для «{baseline_prop_title}» не найдены или пусты.*")
        ...
        for contractor_id_str, contractor_data in lot_data_dict.get(JSON_KEY_PROPOSALS, {}).items():
            ...
            if additional_info_dict:
                ...
                lot_specific_md_lines.append(f" Здесь описываются коммерческие условия, указанные {contractor_name_s} в предложении к тендеру {tender_id_val}.\n")
                lot_specific_md_lines.append("  Коммерческие условия могут включать в себя различные аспекты, такие как сроки выполнения, условия оплаты, гарантии и т.д.\n")
                ...
                for key_info, val_info in additional_info_dict.items():
                    lot_specific_md_lines.append(f"{sanitize_text(key_info)}: {sanitize_text(val_info) if val_info is not None else 'нет данных'}.")
                lot_specific_md_lines.append("")
                    ...
                    if is_chapter_f:
                        ...
                        if not pos_name_s.lower().startswith("лот №"): 
                            ...
                            if isinstance(total_costs_ch_dict, dict) and any(total_costs_ch_dict.get(k, 0) != 0 for k in [JSON_KEY_MATERIALS, JSON_KEY_WORKS, JSON_KEY_INDIRECT_COSTS, JSON_KEY_TOTAL]):
                                org_qty_label = f' за объемы подрядчика {contractor_name_s}' if pos_org_qty_cost_val is not None else ''
                                lot_specific_md_lines.append(
                            ...
                            elif not (isinstance(unit_costs_ch_dict, dict) and any(v is not None for v in unit_costs_ch_dict.values())):
                                lot_specific_md_lines.append(f"Подрядчик {contractor_name_s} {label_suffix_str} не указал стоимость.")

                            if pos_org_qty_cost_val is not None and pos_org_qty_cost_val != (total_costs_ch_dict.get(JSON_KEY_TOTAL, 0) or 0):
                                lot_specific_md_lines.append(f"При этом полная стоимость {label_suffix_str} за объемы заказчика составляет {pos_org_qty_cost_val} руб.")
                            
                            if pos_comm_contr_s:
                                lot_specific_md_lines.append(f"Комментарий {contractor_name_s} {label_suffix_str}: {pos_comm_contr_s}")
                            lot_specific_md_lines.append("") 
                    else: 
                        lot_specific_md_lines.append(f"###### {visible_item_idx_num}. **{pos_name_s}**{section_info_display_str}  ")
                        visible_item_idx_num += 1
