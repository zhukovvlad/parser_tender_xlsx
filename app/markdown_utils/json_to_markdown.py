...
def json_to_markdown(data: Dict[str, Any]) -> Tuple[List[str], Dict[str, Optional[str]]]:
    """
    ...
    initial_metadata = {
        k: v for k, v in initial_metadata.items() 
    ...
    for lot_key_str, lot_data_dict in data.get(JSON_KEY_LOTS, {}).items():
        ...
        for contractor_id_str, contractor_data in lot_data_dict.get(JSON_KEY_PROPOSALS, {}).items():
            ...
            if additional_info_dict:
                ...
                md_lines.append(f" Здесь описываются коммерческие условия, указанные {contractor_name_s} в предложении к тендеру {tender_id_val}.\n")
                md_lines.append("  Коммерческие условия могут включать в себя различные аспекты, такие как сроки выполнения, условия оплаты, гарантии и т.д.\n")
                md_lines.append("  Ниже приведены ключевые коммерческие условия, указанные подрядчиком:\n")
                for key_info, val_info in additional_info_dict.items():
                    md_lines.append(f"{sanitize_text(key_info)}: {sanitize_text(val_info) if val_info is not None else 'нет данных'}.")
                md_lines.append("")
            ...
            if contractor_summary_dict:
                md_lines.append(f"#### Общие итоги по предложению {contractor_name_s}\n")
                ...
                if float(indirect_sum_val != 0 or indirect_vat_val != 0): # Проверяем, что не нули
                     md_lines.append(
                ...
                for _, pos_item_data in sorted_positions_list:
                    ...
                    if is_chapter_f:
                        ...
                        if not pos_name_s.lower().startswith("лот №"): 
                            ...
                            if pos_comm_contr_s:
                                md_lines.append(f"Комментарий {contractor_name_s} {label_suffix_str}: {pos_comm_contr_s}")
                            md_lines.append("") 
                    else: 
                        ...
                        if pos_org_qty_cost_val is not None and pos_org_qty_cost_val != tc_total:
                            md_lines.append(f"pos_org_qty_cost_val - {pos_org_qty_cost_val}. tc_total - {tc_total}.")
