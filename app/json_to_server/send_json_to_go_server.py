...
def send_json_to_go_server(data_to_send: Dict[str, Any], server_url: str, api_key: str = None) -> bool:
    """
    ...
    if api_key:
        headers["Authorization"] = f"Bearer {api_key}" # Пример использования Bearer токена
    ...
    try:
        ...
        response = requests.post(server_url, json=data_to_send, headers=headers, timeout=60) # timeout в секундах
        response.raise_for_status()  # Вызовет исключение для HTTP кодов ошибок 4xx/5xx

        print(f"Данные успешно отправлены. Статус ответа сервера: {response.status_code}")
        try:
