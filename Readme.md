...
### AI результаты → Go → lots.lot_key_parameters

После успешной AI-обработки Python-сервис отправляет результат в Go-сервис, который сохраняет
его в колонку `lots.lot_key_parameters`.
...
# GO_SERVER_AI_RESULTS_ENDPOINT_TEMPLATE=http://localhost:8080/api/v1/tenders/{tender_id}/lots/{lot_id}/ai-results
```

Payload по умолчанию (может быть адаптирован сервером):
...
```json
{
  ...
  "lot_key_parameters": {
    "ai": {
      ...
      "data": { "depth": "4.2 м", "volume": "2850 м³" },
      "processed_at": "2025-08-22T12:34:56Z"
    }
  }
}
```

Если Go временно недоступен, payload сохраняется локально в `pending_sync_json/ai_results/` для
