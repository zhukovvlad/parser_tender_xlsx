...
2.  **Постобработка JSON:** Полученный JSON-файл далее обрабатывается функциями из `postprocess.py`. Эти функции выполняют:
...
* `helpers/`: Директория, содержащая вспомогательные модули для `parse.py`, каждый из которых отвечает за парсинг определенной части документа.
* `postprocess.py`: Скрипт или модуль, содержащий функции для постобработки JSON-данных, сгенерированных `parse.py`.
...
2.  **Постобработка "сырого" JSON:**
    Предполагается, что функции из `postprocess.py` будут вызваны программно для обработки JSON, полученного на шаге 1. Пример вызова (если `postprocess.py` содержит основную функцию для этого):
    ```python
    # Пример псевдокода для вызова постобработки
    # import json
    ...
    ```
    Вам нужно будет реализовать способ вызова этих функций в соответствии с вашей общей архитектурой.
...
### `helpers/` (вспомогательные модули для `parse.py`)
...
* **`normalize_lots_json_structure(data)`**:
    ...
    * Переиндексирует ключи для остальных предложений подрядчиков (например, "contractor_1", "contractor_2").
...
После выполнения парсинга (`parse.py`) и всех шагов постобработки (функциями из `postprocess.py`), итоговый JSON-файл будет иметь следующую структуру (основные моменты):
...
* **CLI для постобработки:** Создание интерфейса командной строки для `postprocess.py`, если требуется его отдельный запуск.
